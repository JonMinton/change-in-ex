...
Next, we display a table with those countries where the breakpoint shows as statistically significant compared to the null, where best_tau is the year.

```{r}
stat_sig_table <- select(tmp, code, x, sex, best_tau, stat_sig_preference) %>% filter(stat_sig_preference==TRUE)

rename(stat_sig_table, age = x, country = code)
view(stat_sig_table)
